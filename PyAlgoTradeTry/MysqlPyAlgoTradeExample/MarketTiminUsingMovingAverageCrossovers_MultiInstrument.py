...
example for using mysql feed function with multiple instruments strategy
@author: Daytona
'''
...
def main(plot):
    ...
    feed = yahoofinance.build_feed(instruments, 2007, 2009, "data", skipErrors=True)
    print feed
    ...
    df = query.pandasQueryMultiple(cnx.connect())
    #close connection
    cnx.close_connection()
    
    # feed data for PyAlgoTrade back testing
    print instruments
    feed = MyLibrary.pandas_feed_mysql.DataFrameBarFeed(df, instruments, barfeed.Frequency.DAY)
