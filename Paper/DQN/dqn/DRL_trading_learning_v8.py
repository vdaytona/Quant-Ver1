...
v8: 1. Add test part for next 10, 20, 50 100 time point at each 10 epoch
@author: Daytona
import os
...
class Trading_Memory():
    ...
    def get_batch(self, target_model,online_model, batch_size=30):
        ...
        for i in range(len(targets_predict_batch)) :
            targets_predict_batch[i, action_batch[i]] = reward_batch[i] + self.__discount * Q_max[i]
            
        return state_batch, targets_predict_batch

class test():
    def __init__(self, look_back_term, transaction_cost,test_data,model=None):
        self.__look_back_term = look_back_term
        self.__transaction_cost = transaction_cost
        self.__model = model
        self.__test_data= test_data
        self.__state_batch = self.get_batch_state()
        
    def set_model(self,model):
        self.__model = model
    
    def get_batch_state(self):
        state_batch = np.zeros((len(self.__test_data) -2 -self.__look_back_term +1 , self.__look_back_term))
        for t in range(self.__look_back_term - 1 , len(self.__test_data) - 2) :
            state = np.zeros((1,self.__look_back_term))
            state[0] = self.__ret_train[t - self.__look_back_term +  1 : t + 1]
            state_batch[t-(self.__look_back_term-1) : t-self.__look_back_term+2] = state
        return state_batch
    ...
    def run_test(self):
        ...
        win_ratio_list = []
        accumulate_pips = [0.0]
        Q_predict = self.__model.predict(self.__state_batch,batch_size = len(self.__state_batch))
        action_batch = Q_predict.argmax(axis = 1)
        for t in range(self.__look_back_term - 1 , len(self.__test_data) - 2) :
            i = t-(self.__look_back_term-1)
            if self.__test_data[t+1] * ACTION_LIST[action_batch[i]] > 0 :
                win += 1
                win_result_list.append(1)
            elif self.__test_data[t+1] * ACTION_LIST[action_batch[i]] < 0 :
                loss += 1
                win_result_list.append(-1)
            else :
                win_result_list.append(0)
            pips = ACTION_LIST[action_batch[i]] * (self.__test_data[t+1]) - \
            self.__transaction_cost * abs(ACTION_LIST[action_batch[i-1]] - ACTION_LIST[action_batch[i]])
            accumulate_pips.append(accumulate_pips[-1]+pips)
        ...
        return win_ratio_list, accumulate_pips 
...
def run():
    ...
    ret_test = (close[1:] - close[:-1])[training_period_stop - look_back_term : training_period_stop + 100]
    ...
    # create test module
    model_test = test(look_back_term, transcation_cost = 0.00025, test_data = ret_test)
    ...
    for e in range(epoch):
        ...
        if e % step_size == 0:
            ...
            target_model = read_model(version, time_start)
            if e > 1 :
                # test at each 10 epoch
                model_test.set_model(model)
                win_ratio_list, accumulate_pips  = model_test.run_test()
                print win_ratio_list
                print accumulate_pips
