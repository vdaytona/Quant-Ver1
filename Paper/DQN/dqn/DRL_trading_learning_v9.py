...
v9: 1. Using convolution1D network    
@author: Daytona
import os
...
class Trading_Memory():
    ...
    def get_batch(self, target_model,online_model, batch_size=30):
        ...
        # have to change the shape (x,x) to (x,x,1)
        joined_batch = np.concatenate((state_batch, new_state_batch))
        model_joined_batch = joined_batch.reshape(joined_batch.shape + (1,))
        joined_predict_batch = target_model.predict(model_joined_batch,batch_size = len(state_batch) * 2)
        targets_predict_batch = joined_predict_batch[: len(joined_predict_batch) / 2]
        
        model_new_state_batch = new_state_batch.reshape(new_state_batch.shape + (1,))
        Q_predict = online_model.predict(model_new_state_batch,batch_size = len(new_state_batch))
        action_next_bath = Q_predict.argmax(axis = 1)
        model_joined_predict_batch = joined_predict_batch.reshape(joined_predict_batch.shape + (1,))
        Q_max = model_joined_predict_batch[len(joined_predict_batch) / 2 : ][np.array(range(len(new_state_batch))), action_next_bath]
...
def run():
    ...
    model = Sequential()
    model.add(Convolution1D(look_back_term, 3, border_mode='same', input_shape=(look_back_term,1),activation=act_function))
    model.add(Convolution1D(look_back_term, 3, border_mode='same',activation=act_function))
    #model.add(MaxPooling1D(pool_length=2, stride=None, border_mode='same'))
    #model.add(Dropout(0.25))
    model.add(MaxPooling1D(pool_length=2))
    model.add(Flatten())
    model.add(Dense(look_back_term, activation=act_function))
    model.add(Dense(num_actions))
    model.compile(sgd(lr=learning_rate), "mse")
    ...
    for e in range(epoch):
        ...
        for t in range(look_back_term - 1 , len(ret_train) - 2) :
            ...
            model_state = state.reshape(state.shape + (1,))
            # decide action
            if np.random.rand() < epsilon:
                action = np.random.randint(0, num_actions, size=1)
            else:
                q = target_model.predict(model_state)
                action = np.argmax(q[0])
            
            new_state, reward = env.act(t, action)
            model_new_state = new_state.reshape(new_state.shape + (1,))
            ...
            model_inputs = inputs.reshape(inputs.shape + (1,))
            model.train_on_batch(model_inputs, targets)
